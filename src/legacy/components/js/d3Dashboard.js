...
var termBorderColors = [];
var termBorderColorsDict = {};
// window.addEventListener("resize", function(event) {
//   updateWindow();
// });

function addFlujoMaterias(programStructure) {
  let flujoMaterias = programStructure.terms
    .reduce(function(prev, actual) {
      return prev.concat(actual.courses);
    }, [])
    .reduce(function(prev, actual) {
      actual.requisites.forEach(function(d) {
        prev[d] ? prev[d].push(actual.code) : (prev[d] = [actual.code]);
      });
      return prev;
    }, {});

  programStructure.terms.forEach(function(term) {
    term.courses.forEach(function(course) {
      course.flujoMaterias = flujoMaterias[course.code]
        ? flujoMaterias[course.code]
        : [];
    });
  });
}
...
function createGraphic(node, programStructure, studentInfoAcademic) {
  nodeGlobal = node;
  addFlujoMaterias(programStructure);
  setTrackingData("coursesOpen", _.join(openCourses, "|"));
  nodeElement = node;
  config = Object.assign({}, originalConfig);
  program = programStructure;
  studentInfo = studentInfoAcademic;

  termBorderColors = _.split(
    _.get(studentInfo, ["student_dropout", "weight_per_semester"], ""),
    ","
  );
  ...
  function addTermsButtons(nestTerms, yPosition) {
    ...
    let buttonWidth = parseInt(config.spacingBetweenTermsButtons / 1.2);
    let buttonHeight = buttonWidth / 2.0;
    const div = d3
      .select(nodeGlobal)
      .append("div")
      .attr("class", "tooltipSituation")
      .style("opacity", 0)
      .style("font", config.situationFont + "px sans-serif");
    ...
    let buttons = svg
      ...
      .on("click", function(d, i) {
        ...
        svg
          ...
          .select("rect")
          .attr("stroke", (d, i) => {
            let color = _.get(termBorderColorsDict, d.key);
            return `rgb(${color},${color},${color})`;
          })
          .attr("stroke-width", config.buttonTermStrokeWidth);

        d3.selectAll(".psp-circle").attr("fill", function(d, i) {
          return d3.select(this).attr("originalColor");
        });

        d3.selectAll(".botonesTerm")
          .filter(function() {
    ...
    buttons
      ...
      .attr("stroke-width", config.buttonTermStrokeWidth)
      .attr("stroke", (d, i) => {
        const termBorderColor = _.get(termBorderColors, i);
        let color = 125;
        if (termBorderColor) {
          const colorBase = 145;
          color = colorBase - (255 - colorBase) * _.toNumber(termBorderColor);
        }
        termBorderColorsDict[d.key] = color;
        return `rgb(${color},${color},${color})`;
      })
    ...
    buttons
      ...
      .attr("y", parseInt(buttonWidth * 0.32))
    ...
    buttons
      .filter(function(d, i) {
        ...
        let situation = d.values[0].situation;
        return situation != null && /\w+:\w+/.test(situation);
      })
      .append("circle")
      .attr("r", config.situationRadius)
      .attr("cx", buttonWidth / 2)
      .attr("cy", buttonHeight - config.situationRadius * 2)
      .attr("fill", function(d) {
        return d.values[0].situation.split(":")[0] === "ELIMINADO"
          ? config.situationColor1
          : config.situationColor2;
      })
      .on("mouseover", function(d) {
        let text = d.values[0].situation.split(":")[1];
        div
          .transition()
          .duration(200)
          .style("opacity", 0.9);
  ...
  function autoResize(node) {
    ...
    let newtextSubjectCodeFontSize = parseInt(
      (newboxWidth + newboxHeight) /
        ((config.boxWidth + config.boxHeight) / config.textSubjectCodeFontSize)
    );
    ...
    let newhistogramNumberFontSize = parseInt(
      newboxWidth / (config.boxWidth / config.histogramNumbersFontSize)
    );
    ...
    config.textSubjectCodeFontSize = newtextSubjectCodeFontSize;
    ...
    config.histogramNumbersFontSize = newhistogramNumberFontSize;
...
function updateWindow() {
  if (config && config.autoResize) {
    ...
    createGraphic(nodeElement, program, studentInfo);
  }
}
...
function unfocusSubjectsByTerm() {
  ...
  ExpandedcourseSVG.select(".registration").text(function(d) {
    if (d) {
      return d[Object.keys(d)[Object.keys(d).length - 1]].registration;
    }
  });
...
function unfocus(subjects, boolean) {
  if (boolean) {
    ...
    subjects.selectAll(".subjectName").attr("opacity", 0.2);
    subjects.selectAll(".subjectCode").attr("opacity", 0.2);
    subjects.selectAll(".registration").attr("opacity", 0.2);
    ...
    subjects.selectAll(".subjectName").attr("opacity", 1);
    subjects.selectAll(".subjectCode").attr("opacity", 1);
    subjects.selectAll(".registration").attr("opacity", 1);
...
function showRequisites(course, boolean) {
  ...
  function show(courseSVG, boolean) {
    if (boolean) {
      ...
      //.attr("fill",config.backgroundColor)
      courseSVG.select("path").attr("stroke", config.requirementStroke);
      ...
      courseSVG.select("path").attr("stroke", config.strokeColor);
...
function showFlujoMaterias(course, boolean) {
  if (boolean) {
    let courseSVG = d3.selectAll(`.subject[flujoMateria='true']`);
    if (!courseSVG.empty()) {
      show(courseSVG, false);
    }

    course.flujoMaterias.forEach(req => {
      let courseSVG = d3.select(`#${req.replace(/\s|:/g, "")}`);
      if (courseSVG.empty()) return;
      show(courseSVG, true);
    });
  ...
  function show(courseSVG, boolean) {
    if (boolean) {
      courseSVG.attr("flujoMateria", "true");
      courseSVG
        .selectAll(".flujoMateriaText")
        .attr("visibility", "visible")
        .attr("transform", function(d, i) {
          if (courseSVG.attr("expanded") === "true") {
            let x = d3.select(this).attr("x")
      ...
      courseSVG
        .select("rect")
        .attr("stroke", config.flujoStroke)
        .attr("stroke-width", function() {
          return courseSVG.attr("expanded") == "false"
            ? config.flujoStrokeWidth
            : config.flujoStrokeWidth / 2;
        });
      // .attr("fill",config.backgroundColor)
      courseSVG.select("path").attr("stroke", config.flujoStroke);
      //courseSVG.selectAll('.grade').attr("visibility","hidden");
      //courseSVG.selectAll('.reproved').attr("visibility","hidden");
    } else {
      courseSVG.attr("flujoMateria", "false");
      //If the requisite course hasnt been clicked
      courseSVG.selectAll(".flujoMateriaText").attr("visibility", "hidden");
      courseSVG
        .select("rect")
        .attr("stroke", config.strokeColor)
        .attr("stroke-width", config.strokeWidth / 2);
...
function focusSubject(node, boolean) {
  if (boolean) {
    ...
    let subjects = d3
      .selectAll(".subject[req='false'][flujoMateria='false']")
      .filter(function(d, i) {
...
function addSubject(node, x, y, width, height, course) {
  ...
  g.append("path")
    ...
    .attr("fill", config.backgroundColor)
    .attr("d", () => {
      return `M${config.borderRadious},0 H${width - bandWidth} V${height} H${
        config.borderRadious
      } A${config.borderRadious},${config.borderRadious} 0 0 1 0,${height -
        config.borderRadious} V${config.borderRadious} A${
        config.borderRadious
      },${config.borderRadious} 0 0 1 ${config.borderRadious},0 Z`;
    });
  ...
  // Subject name
  let subjectText = g.append("text").attr("class", "subjectName");
  let subjectCode = g.append("text").attr("class", "subjectCode");
  wrapText(
    subjectCode,
    ...
    config.textSubjectPadding,
    config.textSubjectCodeFontSize,
    config.textSubjectCodeFontWeight,
    course.code
  );
  wrapText(
    subjectText,
    ...
    width - bandWidth - config.textSubjectPadding * 2,
    config.textSubjectPadding,
    config.textSubjectCodeFontSize + config.textSubjectPadding * 2,
    config.textSubjectFontSize,
    config.textSubjectFontWeigh,
    course.name
  );
  ...
  g.append("circle")
    .attr("stroke", config.flujoStroke)
    .attr("class", "flujoMateriaText")
    .attr("cx", width - bandWidth * 2)
    .attr("cy", height - gradeCircleSize * 2)
    .attr("fill", config.flujoBackground)
    .attr("r", gradeCircleSize)
    .attr("visibility", "hidden");
  ...
  g.append("text")
    ...
    .attr(
      "x",
      (width - bandWidth * 2 - gradeFontSize / 1.2) * config.expandibleFactorX
    )
    .attr("y", config.textSubjectCodeFontSize + config.textSubjectPadding * 2)
    .classed("registration", true)
    .classed("unselectable", true);
  ...
  function onclick(nodeCourse, course) {
    ...
    d3.selectAll(`.botonesTerm:not([id='${termSelected}'])`)
      .select("rect")
      .attr("stroke", (d, i) => {
        let color = _.get(termBorderColorsDict, d.key);
        return `rgb(${color},${color},${color})`;
    ...
    if (nodeCourse.attr("expanded") == "false") {
      openCourses.push(course.code);
      setTrackingData("coursesOpen", _.join(openCourses, "|"));
      Track("click", "open-course", course.code);

      showRequisites(course, true);
      showFlujoMaterias(course, true);
      ...
      if (nodeCourse.data()[0]) {
        ...
        nodeCourse.select(".registration").text(function(d) {
          let term =
            termSelected === ""
              ? nodeCourse.data()[0][Object.keys(d)[Object.keys(d).length - 1]]
                  .registration
              : nodeCourse.data()[0][termSelected].registration;
          return term;
        });
        //console.log(nodeCourse.data()[0][termSelected]);
      ...
      openCourses.splice(openCourses.indexOf(course.code), 1);

      setTrackingData("coursesOpen", _.join(openCourses, "|"));
      Track("click", "close-course", course.code);
      showRequisites(course, false);
      showFlujoMaterias(course, false);
      showHistogram(nodeCourse, false);
      focusSubject(nodeCourse, false);
      if (termSelected != "") focusSubjectsByTerm(termSelected);
      nodeCourse.select(".registration").text("");
...
function addHistogram(node, y, className, term, distribution, grade, title) {
  let zeroTest = distribution.every(element => {
    return element === 0;
  });
  distribution = zeroTest ? [] : distribution;
  ...
  g.selectAll(".tick")
    .selectAll("text")
    .attr("font-size", config.histogramNumbersFontSize);
...
function addReprovedCircles() {
  ...
  /*d3.selectAll(".subject")
    .filter(function(){//Subjects which the studendt took more than once
      let data = d3.select(this).data()[0];
...
function getGradeColor(grade, status) {
  const reprovedGrade = 3.5;
  const approvedGrade = 4.5;
  ...
  if (status === "R") {
    return grade < reprovedGrade
      ? config.rangeGrades[0].color
      : config.rangeGrades[1].color;
  } else if (status === "A") {
    return grade >= approvedGrade
      ? config.rangeGrades[config.rangeGrades.length - 1].color
      : config.rangeGrades[config.rangeGrades.length - 2].color;
  } else if (status === "C") {
    return config.inCourseSubjectColor;
  }
  /*config.rangeGrades.forEach((range)=>{
              color = (grade >= range.minGrade && grade <= range.maxGrade) ? range.color : color;
            });*/
  return config.canceledSubjectColor;
}
